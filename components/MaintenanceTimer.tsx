...
const MaintenanceTimer: React.FC<MaintenanceTimerProps> = ({ user, vehicles, initialSessionId, onBack }) => {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      // Usamos uma versão local atualizada das pausas caso tenhamos acabado de fechar uma
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex items-center justify-between bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-xl font-black text-[#0A2540] tracking-tight uppercase">Gestão da Oficina</h2>
        </div>
        {currentSession || isStartingNew ? (
          <button onClick={() => { setCurrentSession(null); setIsStartingNew(false); fetchActiveSessions(); }} className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-transparent hover:border-slate-300 transition-all">Ver Painel Geral</button>
        ) : (
          <button onClick={() => setIsStartingNew(true)} className="px-5 py-2.5 bg-red-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-red-100 active:scale-95 flex items-center gap-2 transition-all">
      ...
      {!currentSession && !isStartingNew ? (
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {activeSessions.map(s => (
                <div 
                  ...
                  className={`bg-white p-8 rounded-[2.5rem] border-4 transition-all cursor-pointer hover:shadow-xl active:scale-[0.98] ${s.status === 'PAUSED' ? 'border-orange-100 shadow-orange-50' : 'border-[#0A2540] shadow-blue-50'}`}
                >
                   ...
                   <p className="text-[10px] font-bold text-slate-400 uppercase mb-4 truncate">{s.opening_reason}</p>
              ...
              {activeSessions.length === 0 && (
                <div className="col-span-full h-48 border-4 border-dashed border-slate-100 rounded-[3rem] flex flex-col items-center justify-center text-center p-8">
                  <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Nenhuma ordem de serviço ativa</p>
                  <button onClick={() => setIsStartingNew(true)} className="mt-4 text-[10px] font-black text-[#1E90FF] underline uppercase">Abrir Nova Agora</button>
                </div>
              )}
           </div>
        </div>
      ) : isStartingNew ? (
        <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 space-y-8 animate-in zoom-in-95 duration-300">
           <div className="space-y-4">
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Abrir Nova Ordem de Manutenção</h3>
              <div className="space-y-6">
                 ...
                 <div className="space-y-2">
                    <label className="text-[9px] font-black text-slate-300 uppercase tracking-widest ml-2">Motivo da Abertura (Obrigatório)</label>
                    <textarea 
                      value={openingReason} 
                      onChange={e => setOpeningReason(e.target.value)}
                      placeholder="Relate o problema ou o serviço a ser feito..."
        ...
        <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
           <div className="bg-[#0A2540] p-10 rounded-[3rem] shadow-2xl text-center relative overflow-hidden">
              <div className="absolute top-0 left-0 w-full h-1 bg-red-600/30"></div>
              <p className="text-[10px] font-black text-white/40 uppercase tracking-[0.4em] mb-4">Tempo Efetivo de Reparo - {currentSession.prefix}</p>
              <h1 className="text-7xl font-tech font-black text-white tracking-tighter mb-4">
                {formatDuration(calculateEffectiveSeconds(currentSession, pauses, now))}
              </h1>
              <div className="flex items-center justify-center gap-4">
                 <span className={`w-3 h-3 rounded-full ${currentSession.status === 'ACTIVE' ? 'bg-[#58CC02] shadow-[0_0_10px_#58CC02] animate-pulse' : 'bg-orange-500 shadow-[0_0_10px_#f97316]'}`}></span>
           ...
           <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 space-y-6">
              <div className="flex justify-between items-start border-b border-slate-50 pb-6">
                 <div>
                    <h3 className="text-2xl font-black text-[#0A2540]">{currentSession.prefix}</h3>
                 ...
                 <div className="bg-red-50 px-4 py-2 rounded-xl text-right">
                    <span className="text-[10px] font-black text-red-600 uppercase tracking-widest">Técnico Responsável</span>
                    <p className="text-[11px] font-black text-slate-700 uppercase">{currentSession.user_name || user.name}</p>
                 </div>
              </div>
              
              <div className="space-y-2">
                 <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Descrição do Serviço:</p>
                 <p className="text-sm font-bold text-slate-700 leading-relaxed italic">"{currentSession.opening_reason}"</p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
                 {currentSession.status === 'ACTIVE' ? (
                   <button 
                    ...
                    disabled={isSaving}
                    className="py-5 bg-orange-500 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-orange-100 active:scale-95 transition-all flex items-center justify-center gap-3"
                   >
                   ...
                   <button 
                    ...
                    disabled={isSaving}
                    className="py-5 bg-[#58CC02] text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-green-100 active:scale-95 transition-all flex items-center justify-center gap-3"
                   >
                 ...
                 <button 
                  ...
                  disabled={isSaving}
                  className="py-5 bg-[#0A2540] text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg active:scale-95 transition-all flex items-center justify-center gap-3"
                 >
                   {isSaving ? 'Finalizando...' : 'Finalizar e Gravar'}
                 </button>
              </div>
           </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-[#0A2540]/60 backdrop-blur-md">
           <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl max-w-sm w-full space-y-8 animate-in zoom-in-95 duration-200">
              <div className="text-center space-y-2">
                 ...
                 <p className="text-slate-400 font-bold text-xs uppercase tracking-widest">O tempo efetivo será descontado</p>
              </div>
              ...
              <div className="flex gap-3 pt-4">
                 <button onClick={() => setShowPauseModal(false)} className="flex-1 py-4 bg-slate-100 text-slate-400 rounded-xl font-black text-[10px] uppercase">Sair</button>
