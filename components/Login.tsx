    ...
    try {
      ...
      if (authError) {
        ...
        if (authError.message === 'Email not confirmed') {
          throw new Error('E-mail não confirmado. Verifique sua caixa de entrada ou solicite ao administrador que desative a confirmação no Supabase.');
        }
        throw authError;
      }
      ...
      if (authData.user) {
        ...
        if (!profile) throw new Error('Perfil não encontrado no banco de dados.');
      ...
      <div className="max-w-md w-full relative z-10 animate-in fade-in zoom-in-95 duration-1000">
        <div className="bg-[#0f172a]/95 backdrop-blur-3xl rounded-[2.5rem] p-8 sm:p-12 shadow-[0_0_100px_rgba(0,0,0,0.7)] border border-slate-800/60">
          ...
          <div className="flex flex-col items-center mb-10 text-center">
            <h1 className="text-4xl sm:text-5xl font-black tracking-tighter flex items-baseline mb-2">
              <span className="text-[#00548b]">FLEET</span>
              <span className="text-[#425466]">LOG</span>
            </h1>
            <p className="text-slate-400 font-bold text-[10px] uppercase tracking-[0.4em]">Aqui será a Gestão da sua Frota</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="relative">
                ...
                >
                  {showPassword ? (
                    ...
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l18 18" strokeWidth="2"/></svg>
                  )}
                </button>
              </div>
            </div>
            ...
            {error && (
              <div className="bg-red-500/10 text-red-400 p-4 rounded-xl text-[9px] font-black uppercase text-center border border-red-500/20 leading-relaxed">
                {error}
              </div>
            )}
          ...
          <div className="mt-12 pt-8 border-t border-slate-800/80 flex flex-col items-center gap-6 text-center">
            <p className="text-slate-400 font-bold text-[10px] italic opacity-90 border-l-2 border-[#00548b] pl-3">
